...
import { londonToday, clockLabel } from '@/lib/booking-engine/time'
import { summariseAction, type Action } from '@/lib/assistant/command'
import type { Service, BusinessHours } from '@/lib/booking-engine/types'
...
export async function POST(req: Request) {
  ...
  const [services, hours] = await Promise.all([
    select<Service>('services', { active: 'eq.true', order: 'display_order.asc', limit: 100 }),
    select<BusinessHours>('business_hours', { order: 'weekday.asc', limit: 7 }),
  ])
  const action = await parse(text, services, hours, apiKey)
...
function formatHours(hours: BusinessHours[]): string {
  ...
  return hours.map((h) =>
    h.is_open
      ? `${DAY_ABBR[h.weekday]} ${clockLabel(h.open_min)}–${clockLabel(h.close_min)}`
      : `${DAY_ABBR[h.weekday]} closed`
  ).join(', ')
}

async function parse(text: string, services: Service[], hours: BusinessHours[], apiKey: string): Promise<Action> {
  const { default: Anthropic } = await import('@anthropic-ai/sdk')
  const client = new Anthropic({ apiKey })
  const serviceList = services.map((s) => `${s.slug} (${s.name})`).join(', ')
  const hoursLine = formatHours(hours)
...
Current business hours: ${hoursLine}
